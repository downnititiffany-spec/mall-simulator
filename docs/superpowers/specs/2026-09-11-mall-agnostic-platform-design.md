...
- 日期：2026-09-11
- 状态：**待用户评审**（评审通过后进入 `writing-plans` → 分阶段实现）
...
- 本设计通过后需登记新决策条目（预计 D-023 源画像 owner、D-024 每源仓库命名空间、D-025 ODS 保真与代理键）。
...
- 排序：W1 之后（需要商城有详情页与更多商品才有像样的行为流）。

---

## 11. 未决问题（评审时请一并确认）

1. **源 B 的形态**：P5 先用"纯文件异构 fixture"（便宜、可复现、零外部依赖）；是否要把"真接一个开源商城（litemall，MIT、仅 MySQL，需 JDK 8）"排进本期？建议排到 P5 之后作为加分项。
2. **旧 ODS 数据处置**：结构 v2 生效时，旧 ODS/逐字段列直接丢弃重建（演示数据可重建）是否可接受？
3. **`source_registry` vs 扩展现有 `runtime_profile`**：本设计选前者（§4.1 已给理由），请确认。
4. **P4 看板源切换器**是否需要"跨源对比视图"（同屏两源并列），或只做"切换单个源"。
5. **是否需要一个"数据源管理"页面**（列表 / 激活 / 查看画像 / 校验画像合法性），或先只做 API。
