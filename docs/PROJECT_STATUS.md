...
- 已交付（分支 `feature/v3-development`，**未 merge main**）：S2-01A `26b083d`、S2-01B `76e3032`、S2-02A `23d781b`、S2-02B `a673e32`、S2-03 `beddc3e`、S2-03.1 `849ef66`、S2-04A `a754a0e`、S2-04B `bb383ae`、**S2-05 `3c357fb`（ODS/DIM/DWD 主链：DWD 去重键复合化 + `bdw` 阶段序 `dim` 先 + Kahn 真检环）**、**S2-06 `8b6e8e8`（DWS/ADS：ADS 稳定次序键 `rank_no`/RFM `NTILE` 平局裁决 + 全链真跑取证）**、**S3-01 `1bd16b7`（ADS 画像补 R/F/M 原值与观察窗口 + 修窗口正则被解析器吃转义）**、**S3-02 `5d3ef0e`（ADS 销售趋势补净销售额 + 修语义层守卫 ALTER 盲区，本轮）**；映射核心/干跑预览/真实采集接线/批次重放口径/预览→激活五段、"预览→激活"落库闭环、"输入归属"、"采集面"、以及"DWD 去重键与阶段序"均已提交。另：计数口径修复 `7f49055`、`origin/main` 文档重组同步 `d89bd20`。
...
- **S3-02 本轮明确未做（登记，不属本轮范围）**：`repeat_rate`（设计 §11.3 **L433** 要求「**声明观察期和变体**」，属需先冻结口径的**独立任务**，见 backlog 首行）⇒ 作为下一任务；`ads_category_sale`/`ads_region_sale` 实产（§9.3 L336-337）、`ads_behavior_funnel` 分类/渠道粒度（L329）、`ads_data_quality` 规则版本（L335）仍为未完成项。
...
- 计数口径链（2026-09-16）：当前基线 analytics = **876**、default = **995**、spark = **189**；上一版 875/994/183（S3-01）、872/991/177（S2-06）、872/991/143（S2-05）、871/990/111（S2-04B）、834/953（S2-04A）、821/940（S2-03.1）、801/920（S2-03）、773/892（S2-02B）、772/891、764/883（S2-01B 746 + 18）的来历与 `run-tests.ps1` 汇总行漏计解析器缺陷的修复见 F-26/F-29/F-30/F-31/F-32/F-33/F-34/F-35；`scripts/run-tests.ps1` 登记基线已同步为 876/995/189（spark 由 S2-06 的 `AdsStableOrderSpec` 4 条 + `DwsAdsChainExecSpec` 30 条、S3-01 的 `AdsRfmRawValueSpec` 6 条、S3-02 的 `AdsSaleTrendNetSaleSpec` 6 条推高；analytics 由 S3-01 的 Java 守卫 `MetricAdsCatalogDdlConsistencyTest` 3 条与 S3-02 的 `AiSqlDriftTest` 加性迁移自测 1 条推高，逐套件对账无套件消失）。§测试与验收当前口径表中 default-tests 行（analytics632 … = 751）保留为 2026-09-15 发布轮历史口径，**不再作为当前基线**。
