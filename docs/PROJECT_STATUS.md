...
- 计数口径链（2026-09-16）：当前基线 analytics = **872**、default = **991**、spark = **177**；上一版 872/991/143（S2-05）、871/990/111（S2-04B）、834/953（S2-04A）、821/940（S2-03.1）、801/920（S2-03）、773/892（S2-02B）、772/891、764/883（S2-01B 746 + 18）的来历与 `run-tests.ps1` 汇总行漏计解析器缺陷的修复见 F-26/F-29/F-30/F-31/F-32/F-33；`scripts/run-tests.ps1` 登记基线已同步为 872/991/177（spark 由 S2-06 的 `AdsStableOrderSpec` 4 条 + `DwsAdsChainExecSpec` 30 条推高，逐套件对账无套件消失）。§测试与验收当前口径表中 default-tests 行（analytics632 … = 751）保留为 2026-09-15 发布轮历史口径，**不再作为当前基线**。
...
| `ads_user_profile` 缺 R/F/M **原值与观察窗口列**（设计 §11.4 L447 要求"记录原值、score、segment、窗口、rule_version"），DWS `dws_user_trade_period` 已保存这些列 | development backlog，补齐跨 Hive DDL/`MetricAdsSpec`/Java 清单/MySQL 镜像/Flyway 五处 ⇒ 属门 ②/⑥ 邻域，**只登记不擅改**，见 S2-06 差异请求请求 1 |
