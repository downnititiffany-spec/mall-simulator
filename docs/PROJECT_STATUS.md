...
- 计数口径链（2026-09-16）：当前基线 analytics = **871**、default = **990**；上一版 834/953（S2-04A）、821/940（S2-03.1）、801/920（S2-03）、773/892（S2-02B）、772/891、764/883（S2-01B 746 + 18）的来历与 `run-tests.ps1` 汇总行漏计解析器缺陷的修复见 F-26/F-29/F-30/F-31；`scripts/run-tests.ps1` 登记基线已同步为 871/990。§测试与验收当前口径表中 default-tests 行（analytics632 … = 751）保留为 2026-09-15 发布轮历史口径，**不再作为当前基线**。
