...
- 已交付（分支 `feature/v3-development`，**未 merge main**）：S2-01A `26b083d`、S2-01B `76e3032`、S2-02A `23d781b`、S2-02B `a673e32`、S2-03 `beddc3e`、S2-03.1 `849ef66`、**S2-04A `a754a0e`（Landing 输入清单按源 fail-closed，本轮）**；映射核心/干跑预览/真实采集接线/批次重放口径/预览→激活五段、"预览→激活"落库闭环、以及"输入归属"均已提交。另：计数口径修复 `7f49055`、`origin/main` 文档重组同步 `d89bd20`。
...
- 计数口径链（2026-09-16）：当前基线 analytics = **834**、default = **953**；上一版 821/940（S2-03.1）、801/920（S2-03）、773/892（S2-02B）、772/891、764/883（S2-01B 746 + 18）的来历与 `run-tests.ps1` 汇总行漏计解析器缺陷的修复见 F-26/F-29/F-30；`scripts/run-tests.ps1` 登记基线已同步为 834/953。§测试与验收当前口径表中 default-tests 行（analytics632 … = 751）保留为 2026-09-15 发布轮历史口径，**不再作为当前基线**。
