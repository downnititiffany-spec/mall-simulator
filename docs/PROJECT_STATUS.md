...
- 已交付（分支 `feature/v3-development`，**未 merge main**）：S2-01A `26b083d`、S2-01B `76e3032`、S2-02A `23d781b`、S2-02B `a673e32`、S2-03 `beddc3e`、S2-03.1 `849ef66`、S2-04A `a754a0e`、S2-04B `bb383ae`、**S2-05 `3c357fb`（ODS/DIM/DWD 主链：DWD 去重键复合化 + `bdw` 阶段序 `dim` 先 + Kahn 真检环）**、**S2-06 `8b6e8e8`（DWS/ADS：ADS 稳定次序键 `rank_no`/RFM `NTILE` 平局裁决 + 全链真跑取证）**、**S3-01 `1bd16b7`（ADS 画像补 R/F/M 原值与观察窗口 + 修窗口正则被解析器吃转义，本轮）**；映射核心/干跑预览/真实采集接线/批次重放口径/预览→激活五段、"预览→激活"落库闭环、"输入归属"、"采集面"、以及"DWD 去重键与阶段序"均已提交。另：计数口径修复 `7f49055`、`origin/main` 文档重组同步 `d89bd20`。
...
- 计数口径链（2026-09-16）：当前基线 analytics = **875**、default = **994**、spark = **183**；上一版 872/991/177（S2-06）、872/991/143（S2-05）、871/990/111（S2-04B）、834/953（S2-04A）、821/940（S2-03.1）、801/920（S2-03）、773/892（S2-02B）、772/891、764/883（S2-01B 746 + 18）的来历与 `run-tests.ps1` 汇总行漏计解析器缺陷的修复见 F-26/F-29/F-30/F-31/F-32/F-33/F-34；`scripts/run-tests.ps1` 登记基线已同步为 875/994/183（spark 由 S2-06 的 `AdsStableOrderSpec` 4 条 + `DwsAdsChainExecSpec` 30 条、S3-01 的 `AdsRfmRawValueSpec` 6 条推高；analytics 由 S3-01 的 Java 守卫 `MetricAdsCatalogDdlConsistencyTest` 3 条推高，逐套件对账无套件消失）。§测试与验收当前口径表中 default-tests 行（analytics632 … = 751）保留为 2026-09-15 发布轮历史口径，**不再作为当前基线**。
