...
- 已交付（分支 `feature/v3-development`，**未 merge main**）：S2-01A `26b083d`、S2-01B `76e3032`、S2-02A `23d781b`、S2-02B `a673e32`、S2-03 `beddc3e`、S2-03.1 `849ef66`、S2-04A `a754a0e`、S2-04B `bb383ae`、**S2-05 `3c357fb`（ODS/DIM/DWD 主链：DWD 去重键复合化 + `bdw` 阶段序 `dim` 先 + Kahn 真检环）**、**S2-06 `8b6e8e8`（DWS/ADS：ADS 稳定次序键 `rank_no`/RFM `NTILE` 平局裁决 + 全链真跑取证）**、**S3-01 `1bd16b7`（ADS 画像补 R/F/M 原值与观察窗口 + 修窗口正则被解析器吃转义）**、**S3-02 `5d3ef0e`（ADS 销售趋势补净销售额 + 修语义层守卫 ALTER 盲区）**、**S3-03 `b896d83`（ADS 概览落地有效复购率 `repeat_rate` + 观察期声明 + DWS `valid_order_count`，本轮）**；映射核心/干跑预览/真实采集接线/批次重放口径/预览→激活五段、"预览→激活"落库闭环、"输入归属"、"采集面"、以及"DWD 去重键与阶段序"均已提交。另：计数口径修复 `7f49055`、`origin/main` 文档重组同步 `d89bd20`。
...
- 计数口径链（2026-09-16）：当前基线 analytics = **880**、default = **999**、spark = **195**；上一版 876/995/189（S3-02）、875/994/183（S3-01）、872/991/177（S2-06）、872/991/143（S2-05）、871/990/111（S2-04B）、834/953（S2-04A）、821/940（S2-03.1）、801/920（S2-03）、773/892（S2-02B）、772/891、764/883（S2-01B 746 + 18）的来历与 `run-tests.ps1` 汇总行漏计解析器缺陷的修复见 F-26/F-29/F-30/F-31/F-32/F-33/F-34/F-35/F-36；`scripts/run-tests.ps1` 登记基线已同步为 880/999/195（spark 由 S2-06 的 `AdsStableOrderSpec` 4 条 + `DwsAdsChainExecSpec` 30 条、S3-01 的 `AdsRfmRawValueSpec` 6 条、S3-02 的 `AdsSaleTrendNetSaleSpec` 6 条、S3-03 的 `AdsRepeatRateSpec` 6 条推高；analytics 由 S3-01 的 Java 守卫 `MetricAdsCatalogDdlConsistencyTest` 3 条、S3-02 的 `AiSqlDriftTest` 加性迁移自测 1 条与 S3-03 的 `MetricPublisherMappingTest` 4 条推高，逐套件对账无套件消失）。§测试与验收当前口径表中 default-tests 行（analytics632 … = 751）保留为 2026-09-15 发布轮历史口径，**不再作为当前基线**。
...
| 设计 §12.5 L528 要求 `mxp` 导出带 **manifest/checksum**，现行 `_export.json` 与 Java `MetricExportManifest` **无任何摘要字段** ⇒ 导出内容不可校验 | development backlog，**加法实现**（Spark CRC32 摘要 → 清单 `checksum` 键 → Java 校验为 BLOCKING），不触碰 `contract-specs/**`（已逐个核对不覆盖 `_export.json`）；方案见 S2-06 差异请求请求 4，待确认后按 TDD 实施 |
...
| `AdsQualityJob` L15 注释写 `ADS_STAGING_SNAPSHOT_ISOLATION BLOCKING`，而 L61 实现为 `ERROR`（注释陈旧；实测该检查在 S2-06 全链中按 `ERROR` 出事实） | development backlog（**注释/文档锚点**问题，不影响判定），本轮未改生产代码 |
| `AdsPublishJob` L90 引「§14.4 第 6 步」而设计 §14.4 L664 实为业务报告（锚点陈旧） | 同上 |
| `dws_region_sale_day` 无 `net` 列（地区销售额缺净额口径，与 `dws_product_sale_day`/`dws_trade_day` 不一致） | development backlog，本轮未改；补列是否属门 ② 需总控确认 |
...
| `R7_ADDED_COLUMNS`（Spark 侧「在产 Hive 补列」登记清单）**只覆盖概览表**，S3-01/S3-02 给 `ads_user_profile`/`ads_sale_trend` 加的列与 S3-03 的 DWS 列都不在其中 | development backlog（在产 Hive `ALTER TABLE … ADD COLUMNS` 属部署事项；清单覆盖面需专门设计，避免与 DDL 派生成第三所有者） |
