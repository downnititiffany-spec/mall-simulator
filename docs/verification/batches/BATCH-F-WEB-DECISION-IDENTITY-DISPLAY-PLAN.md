# Verification Batch Plan — BATCH-F-WEB-DECISION-IDENTITY-DISPLAY

> Archive status: IMMUTABLE SNAPSHOT
> Source: `docs/verification/CURRENT_BATCH.md` at the time Batch F was opened.
...
因此预计总数 217，但最终必须以实际 Node test 汇总为准，不得用推导值代替实测。
...
本批只有 Web 改动，因此不要额外运行 Java default、spark、isolated、3307 或真实浏览器 E2E。
...
github: verification-results:docs/verification/results/BATCH-F-WEB-DECISION-IDENTITY-DISPLAY-RESULT.md
```
