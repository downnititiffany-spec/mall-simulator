...
### V-032 — Metric 写入型 MySQL IT 双库所有权与 provisioning 预收编

- **Implementation baseline**：`3960cca` + `31d5ed5` + `49b82d0`。
...
- **双库 Flyway IT `49b82d0`**：`platform-app` 新增 profile `isolated-analytics-schema` 和 `AnalyticsIsolationFlywayIT`。该类刻意不挂“缺档案则 disabled”的条件：显式选择 profile 就必须有完整环境；其执行顺序是 meta/metric 两连接各自 `verifyBeforeWrite` → `db/meta` / `db/metric` Flyway → 第二次 migrate 各 0 → `runtime_profile` / `metric_snapshot` 物理归属断言。`platform-app -am -DskipTests test-compile` **PASS**；当前无 `V25_IT_*` 时显式 profile 实测 **1 run / 0 failures / 1 error / 0 skipped**，在 `loadContext()` 即 fail-closed，未进入连接/DDL。脚本形态守卫 `AnalyticsIsolationScriptsContractTest` **3/3 PASS**，钉住 opt-in、双库双账号、Flyway 先行及“无 Password CLI 参数”。
- **默认档回归**：脚本契约守卫净增 3 条，量数轮 analytics **1029 (F=1/E=0/S=1) DRIFT +3**，全部落在 platform-common 106→109；基线 1026→1029 后，加入 schema `*IT` 与 profile 的最终 fresh default 仍为 **1029 MATCH / mall 13 MATCH / generator 110 MATCH / 总计 1152 MATCH**。schema IT 未进入默认档；唯一红仍是既有 `IngestionManifestRuntimePatrolTest.realHistoryOnDiskIsUntouched`。
- **Remaining / why PARTIAL**：代码编排已具备，当前只剩**真实环境执行与收编门**。本 MCP 实测 `V25IT_ADMIN_PWD=MISSING`、`V25_IT_META_PASSWORD=MISSING`、`V25_IT_METRIC_PUBLISH_PASSWORD=MISSING`、`MYSQL3307=NOT_LISTENING`；因此尚未真实创建双库/账号、未真实执行两套 Flyway，也未给 `MetricAdsMySqlIT` / `MetricPublisherMySqlIT` 加 `@Tag("it")`，isolated **55** 基线保持不变。待 3307 环境恢复后先跑 `49b82d0` 的 schema gate；只有该 fresh run PASS 后才允许收编并建立新的 isolated 基线。
