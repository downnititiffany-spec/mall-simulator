...
`metric_value` 只写字典码，`period = day:YYYY-MM-DD`；**窗口型指标**（`repeat_rate`，S3-03）
的 `period = window:<period_start>..<period_end>`，未声明窗口时降级为 `day:` 并由发布侧告警。
...
| 11 | `product_heat` | 商品热度 | `behavior` 事件（view/favorite/cart_add）+ `order` 事件 | `product_id`、`behavior_type` | `dws_product_behavior_day.{pv, fav, cart, buy}` | `ads_hot_product.heat_score`（`1·ln(1+PV)+2·ln(1+收藏)+3·ln(1+加购)+5·ln(1+支付件数)`）、`ads_hot_product.rule_version`（S3-07 补：该行按哪一版**业务权重**算的，取值 = 字典 `product_heat.definition_version`） | `ads_hot_product_m.{heat_score, rule_version}`（不进 `metric_value`） | v1 | 商品分析 |
