...
| 6 | `net_sale` | 净销售额 | `payment` + `refund_completed` 事件 | `dwd_order_detail.paid_amount`、`refund_amount` | `dws_trade_day.net_sale_amount` | `ads_operation_overview.net_sale_amount` | `ads_operation_overview_m.net_sale_amount` + `metric_value(net_sale)` | v1 | 运营总览、销售分析 |
