...
>
> **v1.10（2026-09-16，S3-41 加性补充·**读侧消费/展示口径**）**：关闭 backlog 行 L454 的**展示侧**缺口（该行原文称
> 「`cart_rate` … **未进概览 API 返回**」，本轮实测该措辞**已过期**，见本段「契约事实」）；依据契约 **§1 总原则 4**
> 「响应是**图表语义数据**…比值同时给 `decimal` 与单位/百分号语义」、可编辑指标字典
> `docs/contracts/metric-dictionary.md:23-24`（`cart_rate`＝加购用户数÷浏览用户数、`buy_rate`＝支付用户数÷浏览用户数）
> 与指导书 V3.0 §8 阶段5 完成标准 **L201**「员工能完成主要操作并理解结果；页面**不混源/混快照**；无数据不造数」。
>
> **契约事实（把已存在的可见面写进契约；本版不改字段语义、不改后端、不新增响应字段）**：§3.1 `metrics[*]` 是
> **通用**条目（`metricCode`/`metricName`/`value`/`unit`/`period`/`definitionVersion`）；后端 `AnalysisService.metrics()`
> **遍历 `metric_value` 全部行、不按码过滤**，字典缺该码时 `metricName` 留空（**不臆造**）⇒ 发布侧能产出的**每个**码
> 都会出现在 `data.metrics[]`。实测发布侧码集合 ＝ `MetricPublisher.OVERVIEW_TO_METRIC` 的 **12** 个值 ＋ 漏斗整体率
> `buy_rate`/`cart_rate` ⇒ **14** 个；其中 **9** 个是页面固定卡片（`CARD_META`），其余 **5** 个
> （`cart_rate`/`buy_rate`/`full_refund_rate`/`fav_cnt`/`cart_add_cnt`）此前一律落到「清单外指标照实展示」分支并
> 按 **两位小数** 显示（改前实测：`0.2531`⇒`0.25`、`12345`⇒`12,345.00`、标题⇒裸码 `cart_add_cnt`）。
>
> **展示义务（读侧对齐口径）**：
> 1. 「清单外」条目**必须**按该码的**口径量纲**格式化：**比例型**（decimal 比例：`cart_rate`/`buy_rate`/
>    `full_refund_rate`）按 `formatPercent`（§1 总原则 4「比值给 decimal 与百分号语义」）；**计数型**
>    （`fav_cnt`/`cart_add_cnt`）按 `formatInteger`；**未登记**的码照实展示 `formatNumber(v, 2)` —— **不猜**量纲、
>    **不**擅自乘 100。
> 2. 标题取值顺序：**指标字典名**（`metricName`）→ **前端展示名登记表**（仅当字典缺该码时）→ **指标码**；
>    三步**都不得**臆造口径语义。
> 3. 该码**不在** `data.metricDictionary` 时（改前实测：`full_refund_rate`/`fav_cnt`/`cart_add_cnt` 在已发布
>    `V2__platform_pipeline_quality.sql:63-78` 的 `metric_definition` 种子里**无行**），页面**必须**显示
>    「**字典未登记口径**」提示（**字典整体为空**时例外：口径面板已统一说明「当前快照未提供指标口径字典」，
>    此时不逐卡重复同一原因）；**不得**据此推断口径、**不得**在页面写公式（口径唯一 owner 仍是字典）；
>    补字典行属**加性迁移**、需另行裁决（已发布 V2 迁移**不得**改）。
> 4. 唯一属主：清单外条目的量纲/展示名登记表 ＝ `web/src/utils/metricDisplay.js`；视图**不得**自持第二份
>    「码→量纲/名称」映射；固定卡片仍由 `CARD_META` 决定顺序与量纲，二者**不得冲突**。
> 5. 数值**不重算**、空值仍显示占位符（不得显示成 0）；导出 CSV 既有列**不变**。
>
> **勘误（v1.9 引用）**：v1.9 展示义务第 3 条引用的「§1.4 空值规则」在本契约内**不存在该编号**（空引用）；
> 该条**语义不变**，正确依据是设计 §11.2 **L428**「**空值规则**和版本」＋契约 §1 总原则 3「不得编造数值」＋
> 前端 `web/src/utils/number.js` 的空值占位符约定（`EMPTY_TEXT`）。**本版不修改 v1.9 原文**，以勘误条附录。
>
> **仍未实现（不得因本版发布而声称已满足）**：`full_refund_rate`/`fav_cnt`/`cart_add_cnt` 的**字典行**（加性迁移，
> 未做）；「跨树『发布侧可产出码集合 ↔ 前端量纲登记表』自动守卫」（**未做**，本轮登记为候选）；v1.9 注记同列的
> 其余项（`repeat_rate` 口径版本展示、支付复购率变体、E5-c 剩余一半、`/analysis/sales` 真窗口过滤）**均不变**。
...
- **v1.10**：`metrics[*]` 是**通用**条目、后端**不按码过滤**（发布侧可产出 **14** 个码：`OVERVIEW_TO_METRIC` 的 12
  个值 ＋ `buy_rate`/`cart_rate`）；「清单外」条目的量纲展示义务（比例按百分比、计数按整数、未登记码照实展示、
  名称三级回退、字典缺行时提示「字典未登记口径」）见文首 **v1.10**。
