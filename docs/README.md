...
- `项目完整实施指导书 V2.4.md`：三程序边界、商城无关化、集群、数据结构、算法、测试，以及受控 AI 数仓设计规则（**最高实施依据**）。2026-09-12 由 V2.2 迭代出 V2.3（相对 V2.2 的唯一实质变化＝新增 §4.1.1「核心接口的追认与补全」），同日再由 V2.3 迭代出 **V2.4**（相对 V2.3 只改 §4.1.1.3 表内三行口径与 `ProductPage` 第三缺口通道，逐项见 V2.4 文末「版本变更说明（V2.3 → V2.4）」；V2.3 转历史、正文逐字未改，冷备于 `backups/m1-9-review-20260912/`）。
- `项目实施进度与任务看板 V2.2.md`：唯一任务执行入口；记录 P1–P5 与 AIW-001～020 的状态、Owner、依赖、反馈时间和证据。（M1-4 泳道 2026-09-11/12 的证据段写在历史文件 `项目实施进度与任务看板.md` §3.5/§3.9–§3.12；该漂移已由看板 §5.1 裁决 D-049 登记，入口仍是本文件。）
...
- `项目完整实施指导书 V2.1.md`、`项目完整实施指导书 V2.2.md`（2026-09-12 起）、`项目完整实施指导书 V2.3.md`（2026-09-12 起）、`项目实施进度与任务看板.md`、`开发过程事实与决策记录.md`：历史快照，只读追溯；不再作为当前执行入口。
...
4. 当前结论以 **V2.4**、V2.2 看板和可复跑证据为准；269 条统计是 `34f37a8` 的冻结快照，不随代码变化自动更新。
