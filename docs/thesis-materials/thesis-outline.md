...
- 表清单：**路径已实测存在** —— `warehouse/ddl/00-ods.sql`、`01-dwd.sql`、`02-dims.sql`、`03-dws.sql`、`04-ads.sql`（注意第 3 个文件名是 **`02-dims.sql`**，不是 `02-dim.sql`）。
  ...
  - **〔2026-09-12 补记 · P2-07 勘误，以此为准〕** 上一行的"前缀由 `runtime_profile.hive_database_prefix` 决定"已被 D-070 取代：前缀的**唯一事实来源**是 `source_registry.warehouse_prefix`（每源一行、`VARCHAR(24) NOT NULL`、无 DEFAULT，V18 迁移），解析链唯一 = `source_registry.warehouse_prefix` → `WarehouseNamespaceProvider.forSource(sourceId)` → `JobCommandBuilder --hiveDatabasePrefix`；`runtime_profile.hive_database_prefix` 自 2026-09-12 起**只断读、不删列**（写非空值会被 `RuntimeProfileServiceImpl` 直接拒绝，D-073）。上面那句历史文本保留不删，仅在此登记勘误并指向 `docs/acceptance/p2-07-source-prefix-20260912/RULINGS-20260912.md` 的 D-070/D-072/D-073。契约版本亦由 `warehouse-namespace.v1.json` 升为 `v2.json`（D-072；`contract-specs/VERSION` 现为 `2.1.0`，CT 批次已在 2.0.0 之上递增）。
