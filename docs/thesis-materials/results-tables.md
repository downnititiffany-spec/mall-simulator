...
| **D-R9-2** `TradeDwdJob` 维度关联未按生效日期过滤 | `dim_user`/`dim_product` 每日一份快照 → 关联笛卡尔放大：订单明细 7→**28 行**、金额类指标 ×4：GMV 2042.00→**8168.00**、净销售额 1493.00→**5972.00**、客单价 408.40→**1633.60**（去重计数类不变）；该放大值曾随 run 29 发布为 ACTIVE，11:09 被 run 30 覆盖（`33-snapshot-history-values.tsv`） | 两处 JOIN 增加 `AND dt = '$dimDt'` | `dwd_order_detail=7`、`dws_trade_day=5/2042.00/1493.00/408.40`、ACTIVE 指标=黄金（P7/P8） |
