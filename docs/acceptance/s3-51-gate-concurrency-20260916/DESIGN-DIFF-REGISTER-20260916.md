...
4. **被拒运行仍会创建空 `LogDir`**：`New-Item` 在门禁之前。是否延后建目录**未改**（改它会动 `-LogDir` 语义）。**收口补测更正**：本轮两条缺省探针留下的空目录**已清理**（`Test-Path` 实测均 False）；TEMP 现存 `v25tests-dev003c_*` 目录 **25** 个 —— 它们是历史门禁的**证据目录（绝大多数非空）**，**不是**空目录；`v25tests-*` 合计 **35** 个，其中**完全为空者 1 个**＝`v25tests-dev003c_20260915_1147s`（`2026-09-15 11:42`，即上条「被拒/中止运行留空目录」的实例）。
