<#
V26-F88 | 在隔离实例（WSL MySQL 8.0.41 @ 127.0.0.1:3307）上创建本次 runId 命名的库与受限账号。

纪律（与 V25-E3 00-create-isolation.ps1 同源，仅把 runId 形态放宽为「下划线分段」）：
  * 唯一写入目标 = 3307。**建立连接之前**先做端口白名单 + 实例指纹核对；不符立即拒绝，不连接。
  * 3306 一律拒绝（连都不连）。
  * 落盘全部在仓库内且被 gitignore：target/f88-run/<runId>/。
  * 口令随机生成，只写进 (scratch)/credref.properties；docs/ 证据里不出现口令。

用法：pwsh -NoProfile -File <this> -RunId <id>
#>
param(
...
$metaUser = "${token}_meta"
$pubUser  = "${token}_metric_pub"
$readUser = "${token}_metric_read"
foreach ($n in @($metaUser, $pubUser, $readUser)) {
  if ($n.Length -gt 32) { throw "账号名超过 MySQL 32 字符上限: $n ($($n.Length))" }
}
...
Write-Host "  meta user     : $metaUser@'%'   -> $metaDb.*"
Write-Host "  publish user  : $pubUser@'%'    -> $metricDb.*"
...
SELECT 'DB_EMPTY_CHECK', TABLE_SCHEMA, COUNT(*) AS tables_cnt
  FROM information_schema.TABLES WHERE TABLE_SCHEMA IN ('$metaDb','$metricDb')
...
@(
  "# V26-F88 隔离凭据引用（仓库内 target/，gitignore）。口令不写入 docs/ 证据。",
