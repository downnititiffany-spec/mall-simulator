...
## 同日补记（2026-09-12 20:59，append-only，以此为准）

**① 本文件 §2.1 所 pin 的画像指纹已被后续修改取代。** 原记：3,339 B / mtime 20:48:24 / sha256 `FBA70E3C3B1D4D81…`。**现值（父侧 20:58:52 自测）＝ 3,323 B / mtime 2026-09-12 20:49:29 / sha256 `0BB8A05C8D5E466864DCB90B8D7B97105DC65497DC3CCB463D104F27F021170B` / 顶层键 9。** 变更原因：泳道在收到 D-139 之后按 §2.4 自查，抓出**自身 5 处未获授权的归一映射**（`add_cart→cart_add`、`age18_24→18-24` 等）并全部改为 `null`，随后重新落盘。原文一律保留，不改写。**P2-02 开工前必须按新值复测。**

**② §2.4 所指「`null` 是否合法表达」已裁定**：见 `RULINGS-P3-01A-FINAL-20260912.md`（D-140）§3 —— `null` 是**受控扩展**（设计书无此行文），定义＝「该取值已被实测观测到，但平台尚未裁定其规范语义（待裁定）」，与 §4.2 L147 的"省略键＝该源没有这个语义"**不得归一**；落地面 = `P3-01-c`（将该定义逐字追加到设计书 §4.2）。

**③ 状态**：P3-01-a 记 **`DONE_LIMITED`**（理由与解除条件见 D-140 §1），**不得**读作 P3-01 已完成、种子源已激活。