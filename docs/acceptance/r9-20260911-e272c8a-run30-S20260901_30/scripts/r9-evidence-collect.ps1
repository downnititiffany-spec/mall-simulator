...
$desc = @{
  ...
  '18-r7-4-dom-report.json' = '平台 8 页真机 DOM 验收（22/22 PASS）'
  '18-r7-4-mall-dom-report.json' = '商城页面真机 DOM 验收（17/17 PASS）'
  '18-r8-accept-report.json' = 'R8 真机验收（A 身份/B 证据包/C 六段解释/D 安全问数/E 攻击集/F 决策，53/53 PASS）'
  '18-r8-evidence-truncation-proof.json' = '阶段证据截断修复复验（12/12 PASS）'
  'hive-evidence.sql' = '第 3 步 Hive 复核所用 SQL 原文'
  '19-spark-jobs.jar' = '产生本次 run 的作业 jar 快照（MD5 可与重建产物比对）'
  ...
  '33-snapshot-history-values.tsv' = 'D-R9-2 直接证据：逐快照指标值（metrics 直读）——修复前 run 29 的 S20260901_29 金额类放大 4 倍并曾为 ACTIVE，11:09 被 run 30 黄金值覆盖'
