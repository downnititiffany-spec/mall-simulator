...
## 补记（2026-09-14，总控）：退出码 5 / 6 语义分离 —— 以此为准

本节为**追加**，不改动上文任何原文（含 F 行对 `scripts/smoke-pipeline.ps1` 的建议）。

* 上文 F 行（`smoke-pipeline.ps1:137/138/149/194` 把 mysql 错误记录强转 `[long]` ⇒ 连不上时崩溃 `exit 1`）给出的建议是「在 `Q()` 内对非零退出码统一 `exit 5`」。
* V25-S02/K-01～K-05 泳道（提交 `98393f0`）实测后**有意改为语义分离**：
  - `exit 5` = 目标/凭据在**执行前**就不明确被拒（口令缺失、库名不在白名单、账号为 root）；
  - `exit 6` = 执行中**只读取数失败/结果不可解析**（mysql 非零退出或输出不可解析）。
* 判断：分离后「执行前拒绝」与「执行中取数失败」在退出码上可区分，优于两者都记 5。**以新契约（`0/2/3/4/5/6`，见 `scripts/smoke-pipeline.ps1:32`）为准。**
* 证据：`docs/acceptance/v25-s02-k01k05-20260914/raw/k05-exitcode-matrix.json`（退出码 5/5/5/6，`failed=0`）；总控复核报告 `docs/acceptance/v25-s02-k01k05-20260914/CONTROLLER-VERIFICATION-20260914.md` §V-7、§V-8、§B-3。
