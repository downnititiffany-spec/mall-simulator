...
# 2026-09-15 S2-02B 收口口径（analytics=772 / default=891）：相对上一版 764/883 只差 +8，
#   全部来自 connection-ingestion 新增的两个用例类（IngestionFailureTaxonomyTest 6 + IngestionBoundaryMatrixTest 2）；
#   其余五个模块逐一复核未变。
#   analytics-server = 772 = 90(platform-common) + 289(connection-ingestion，跳过 1) + 134(warehouse-pipeline)
#     + 48(metric-analysis) + 91(ai-decision) + 120(platform-app)；三树 891 = 772 + 13(mall) + 106(generator)。
