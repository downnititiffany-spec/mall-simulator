 ...
 *
 * <p>为什么单独测接线：`page`/`size`/`topN` 全是可空 {@code Integer}，参数错位**编译器查不出来**，
 * 而服务层测试（{@code AnalysisServiceTest}）直接调服务、绕过了控制器。本类只钉「按位置原样下传 +
 * 缺省不代解析」；分页语义与非法参数（{@code PARAM_INVALID}）由服务层测试覆盖，
 * 不在两处重复断言（避免两个所有者）。</p>
 */
class AnalysisControllerTest {
    ...
    @Test
    @DisplayName("products：page/size/topN 三个可分页参数按位置原样下传，不交换、不代解析")
    void productsForwardsPagingParamsInOrder() {
        ...
        when(analysisService.products("S1", 2, 5, 3, LocalDate.of(2026, 9, 1), LocalDate.of(2026, 9, 2)))
                .thenReturn(model);

        ApiResponse<AnalysisViewModel<ProductsData>> response =
                controller.products(2, 5, 3, LocalDate.of(2026, 9, 1), LocalDate.of(2026, 9, 2), "S1");

        assertThat(response.code()).isEqualTo("OK");
        assertThat(response.traceId()).isNotBlank();
        assertThat(response.data()).isSameAs(model);
        verify(analysisService).products("S1", 2, 5, 3, LocalDate.of(2026, 9, 1), LocalDate.of(2026, 9, 2));
    }

    @Test
    @DisplayName("products：三个分页参数全缺省时传 null（缺省解析的唯一所有者是服务层）")
    void productsPassesNullsWhenPagingParamsAbsent() {
        when(analysisService.products(null, null, null, null, null, null))
                .thenReturn(AnalysisViewModel.empty(java.util.Map.of(), List.of()));

        controller.products(null, null, null, null, null, null);

        verify(analysisService).products(null, null, null, null, null, null);
    }
}
