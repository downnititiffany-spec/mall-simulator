...
test('主按钮在 busy 时仍可点击并切换为显式取消动作', () => {
  assert.match(source, /@click="handleAskAction"\s+:disabled="!busy && !question\.trim\(\)"/)
  assert.match(source, /busy \? '放弃本次分析' : '发送'/)
...
test('同一个主按钮只在 busy 时取消，否则仍发起正常 ask', () => {
...
test('在途分析期间推荐问题和历史回填均禁用，避免输入文字与在途请求错位', () => {
  assert.match(source, /v-for="q in recommended"[^>]*@click="askPreset\(q\)"[^>]*:disabled="busy"/)
  assert.match(source, /v-for="h in history"[^>]*@click="question = h\.question"[^>]*:disabled="busy"/)
})
