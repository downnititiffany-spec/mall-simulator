...
适配器 SPI 的**契约文本**是 `docs/项目完整实施指导书 V2.3.md` §4.1.1（2026-09-12 起，含同日补记）：八个方法以 `TargetConfig` 为首参、
11 条硬约束（其中**路由所有权**【新增要求】：流水与报告的 `http_method`/`route` 必须取自 `MallTargetAdapter.operationRoutes(...)`，
引擎/计划/流水里不得出现商城路由字面量）、DTO 最小字段表，以及"商城状态词 → 规范词"的映射归适配器。

- 新增一家商城 = 新增**一个适配器实现** ＋ 在 `config/GeneratorBeans` 的登记处**加一行**；不得改场景引擎、事件契约、表结构与页面。
- 契约 ↔ 代码的逐条审计、此前未登记的偏离（D13–D16 / R1）与未取证清单见 `docs/acceptance/m1-9-contract-first-20260912/README.md`。
- **2026-09-12**：M1-9 ②（路由所有权 ＋ 第二家商城适配器）开工；完成前本模块进度一律以看板 M1-9 行为准，本节不预告结论。
